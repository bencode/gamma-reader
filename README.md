...
- **Read** PDF, Word, Excel, Markdown, HTML, images, and text in tabs. PDF, Word, and Markdown carry an outline and a reading theme; Word and Markdown also take a text size and page width. [Document support](#document-support) lists what each format offers.
...
Excel support covers `.xlsx` only; the older binary `.xls` format is stored but not read, and a `.csv` opens as text. A sheet is shown as it is stored, without cell colours, charts, or images, and a formula shows the value the spreadsheet saved with it. Very wide sheets are cut off in the preview after the first hundred columns, which the assistant still reads in full.
